...
final class SybaseDialectTest extends TestCase
{
    ...
    public function testApplyPaginationWithOffsetUsesRowNumber(): void
    {
        ...
        $expected = 'SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY id ASC) AS [__row_number], * FROM (SELECT id, name FROM users) __inner) __paged WHERE [__row_number] BETWEEN 21 AND 30';
